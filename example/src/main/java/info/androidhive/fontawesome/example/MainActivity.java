...
public class MainActivity extends AppCompatActivity
    ...
    private TextView skipResultTextView;
    ...
    private void initViews() {
        skipResultTextView = findViewById(R.id.edit_text_skip_result);
        ...
        skipButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                skipResultTextView.setText(R.string.skip_btn_pressed_notification);
            }
        });
    }
    ...
    private void intDrawerLayout() {
        ...
        renderMenuIcons(navigationView.getMenu(), icons, false);

        int[] iconsSubmenu = {R.string.fa_cog_solid, R.string.fa_sign_out_alt_solid};

        // rending icons in second section group
        renderMenuIcons(navigationView.getMenu().getItem(7).getSubMenu(), iconsSubmenu, false);
    }

    /**
     * Looping through menu icons are applying font drawable
     */
    private void renderMenuIcons(Menu menu, int[] icons, boolean isBrand) {
        for (int i = 0; i < menu.size(); i++) {
            MenuItem menuItem = menu.getItem(i);
            if (!menuItem.hasSubMenu()) {
                FontDrawable drawable = new FontDrawable(this, icons[i], true, isBrand);
